//! Footnotes and floats on the same page, against pdflatex.
//!
//! `\@makecol` builds the column with the default
//! `build/column/outputbox` plug (`footnotes-floats-legacy`): the notes
//! attach to `\box\@cclv` first (`\vskip\skip\footins`, `\footnoterule`,
//! `\unvbox\footins`) and only then `\@combinefloats` puts the top floats
//! above (`\@cflt`: `\floatsep` between them, `\textfloatsep` before the
//! body) and the bottom floats below (`\@cflb`: `\textfloatsep` after the
//! notes). The whole column is packed `\vbox to\@colht` once, so a single
//! glue set ratio covers the body's glue, `\skip\footins` and both float
//! separations. On the page-goal side the floats reserve space through
//! `\@colroom` while the `\footins` insertion charges `\skip\footins` and
//! each note against `\pagegoal` (TeX §1008-§1010), and `\@specialoutput`
//! adds `\ht\footins + \skip\footins + \dp\footins` to `\@pageht` before
//! `\@addtocurcol` decides whether a float still fits the column.
//!
//! Expected data: `fixtures/float-notes/expected/*.txt`, every word's
//! origin and baseline, every rule and every image rectangle read from the
//! content streams of pdflatex's own PDF by
//! `tools/float-notes-oracle/generate.py`. **reference_engine: pdfTeX
...
//! `fixtures/floats/`. No TeX runs here.
//!
...
//! rectangle within 0.1bp and 1bp.
...
fn dir() -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures/float-notes")
}
...
#[test]
fn footnotes_and_floats_on_one_page_match_pdflatex() {
    if !lm_available() {
        eprintln!("skipping: Latin Modern not installed");
        return;
    }
    let names = fixtures();
    assert!(names.len() >= 10, "expected at least 10 float-note fixtures, found {}", names.len());
    ...
    for name in &names {
        ...
        eprintln!("{line}");
        if std::env::var_os("FLOAT_NOTES_VERBOSE").is_some() {
