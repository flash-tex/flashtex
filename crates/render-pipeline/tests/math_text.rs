...
fn place(fonts: &FontSet, texts: &[&str], scripts: Option<(&str, &str)>) -> Placed {
    ...
    let tex = TexMathMetrics::new(12, false, false, otf, fonts);
    ...
    let metrics = TextRunMetrics::new(&tex, fonts, fonts.shaper(), Family::LatinModern, false, &sink.texts, &sink.keys, &sink.italics);
