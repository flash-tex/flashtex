...
/// * `10-report-chapter-reset`: the pinned compiler never resets the
///   `footnote` counter at `\chapter` (report/book `\@addtoreset`);
/// * `12-thanks`: the pinned compiler strips `\thanks`;
/// * `13-minipage`: `minipage` footnotes (the box layout is not on main;
///   `footnotes::MinipageNotes` is the hook);
/// * `14-multipar-note`: the pinned compiler closes a `\footnote` argument
///   at a blank line (`\@footnotetext` is `\long`);
/// * `15-math-note`: math in a note is laid out at the body size (the TeX
///   math metrics exist for the class base size only).
const REPORTED: &[&str] = &["10-report-chapter-reset", "12-thanks", "13-minipage", "14-multipar-note", "15-math-note"];
