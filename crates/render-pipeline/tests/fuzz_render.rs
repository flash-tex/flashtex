//! The render-pipeline mutation fuzzer as an ignored test (debug build, so
//! overflow checks and debug assertions are on and stack frames are at their
//! largest).
//!
//! ```text
//! FLASHTEX_FUZZ_CASES=2000 cargo test --test fuzz_render -- --ignored --nocapture
//! ```
//!
//! The test re-executes its own binary as the worker process (see
//! `fuzz_support`). For long runs use the example:
//! `cargo run --release --example fuzz_render`.
...
fn out_dir() -> std::path::PathBuf {
    std::env::temp_dir().join("flashtex-fuzz-render-findings")
}

#[test]
#[ignore = "fuzzing run; minutes long"]
fn fuzz_render_mutations() {
    if let Ok(range) = std::env::var("FLASHTEX_FUZZ_WORKER") {
        ...
        worker(
            ...
            Duration::from_millis(env_u64("FLASHTEX_FUZZ_TIMEOUT_MS", 60_000)),
            &out_dir(),
        );
        return;
    }
    ...
    let config = Config {
        ...
        cases: env_u64("FLASHTEX_FUZZ_CASES", 1000),
        jobs: env_u64("FLASHTEX_FUZZ_JOBS", 4) as usize,
        timeout: Duration::from_millis(env_u64("FLASHTEX_FUZZ_TIMEOUT_MS", 60_000)),
        out_dir: out_dir(),
        max_rss_mb: env_u64("FLASHTEX_FUZZ_MAX_RSS_MB", DEFAULT_MAX_RSS_MB),
    };
    let exe = std::env::current_exe().unwrap();
    let command = |start: u64, end: u64| {
        let mut c = Command::new(&exe);
        c.args(["fuzz_render_mutations", "--exact", "--ignored", "--nocapture", "--test-threads=1"])
