...
fn setlist_calls(source: &str) -> Vec<(&str, &str)> {
    // Indexed once per text for the adapt call: this runs for every list
    // item, and a whole-source scan each time made a 10 000-item list
    // quadratic.
    let ranges = with_source_index(source, |e| e.setlists.get_or_insert_with(|| setlist_ranges(source).into()).clone())
        .unwrap_or_else(|| setlist_ranges(source).into());
    ranges.iter().map(|(envs, keys)| (&source[envs.clone()], &source[keys.clone()])).collect()
}

/// `\setlist[<envs>]{<keys>}`: the byte ranges of `envs` and `keys`.
type SetlistRanges = (std::ops::Range<usize>, std::ops::Range<usize>);

fn setlist_ranges(source: &str) -> Vec<SetlistRanges> {
    let range = |part: &str| {
        let start = part.as_ptr() as usize - source.as_ptr() as usize;
    ...
    while let Some(at) = find_command(&source[from..], "setlist") {
        ...
        calls.push((range(envs), range(&rest[1..close])));
    }
    calls
}
...
fn list_stack_at(source: &str, at: usize) -> Vec<(&str, &str)> {
    ...
    // One pass. Searching `source[from..at]` for the next `\begin` and the
    // next `\end` again after every hit rescanned to `at` whenever one of
    // them was absent: quadratic in the `\begin`s before `at`, and this
    // runs once per list item, so 400 nested lists took 1.1 s to adapt.
    // Every list item asks this, so the scan is indexed once per text for
    // the adapt call (`MacroDefsScope`) and only the part before `at` read.
    for (pos, is_begin) in begin_end_commands_before(source, at) {
...
/// [`begin_end_commands`] of `source[..at]`, from the text's index when a
/// [`MacroDefsScope`] registered it.
fn begin_end_commands_before(source: &str, at: usize) -> Vec<(usize, bool)> {
    match with_source_index(source, |e| e.begin_end.get_or_insert_with(|| begin_end_commands(source).into()).clone()) {
        Some(all) => all[..all.partition_point(|(pos, _)| *pos < at)].to_vec(),
        None => begin_end_commands(&source[..at]),
    }
}

/// Byte offsets of every `\begin` and `\end` in `source`, in order, found
/// exactly as [`find_command`] finds them (comments skipped, a letter after
...
fn begin_end_commands(source: &str) -> Vec<(usize, bool)> {
    ...
    while i < bytes.len() {
        ...
        if c == b'\\' {
            for (needle, is_begin) in [("\\begin", true), ("\\end", false)] {
...
fn in_theorem_environment(text: &str, at: usize, envs: &std::collections::HashSet<String>) -> bool {
    ...
    // Asked for every paragraph: the `\begin`/`\end` scan is indexed once
    // per text (see `list_stack_at`), not redone to `at` after every hit.
    for (pos, is_begin) in begin_end_commands_before(text, at) {
...
struct MacroDefsEntry {
    ...
    /// [`begin_end_commands`] of the whole text, for [`list_stack_at`].
    begin_end: Option<std::rc::Rc<[(usize, bool)]>>,
    /// [`setlist_ranges`] of the whole text, for [`setlist_calls`].
    setlists: Option<std::rc::Rc<[SetlistRanges]>>,
}

/// Runs `f` on the live [`MacroDefsScope`]'s entry for `source`; `None`
/// outside a scope or for a text it did not register.
fn with_source_index<R>(source: &str, f: impl FnOnce(&mut MacroDefsEntry) -> R) -> Option<R> {
