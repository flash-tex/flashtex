...
/// `chapter_starts` are reading positions (`order`, see
/// [`crate::adapter::reading_position`]).
pub fn numbers(found: &[Listing], chapter_starts: &[(usize, String)], order: &[Span]) -> Vec<String> {
    let captioned: Vec<usize> = found
        .iter()
        .filter(|l| l.keys.caption.is_some_and(|(s, e)| s < e))
        .map(|l| crate::adapter::reading_position(order, DocumentId(l.document), l.begin.0).unwrap_or(l.begin.0))
        .collect();
...
pub fn apply(
    ...
    let numbers = numbers(&found, chapter_starts, &labels.reading_order);
