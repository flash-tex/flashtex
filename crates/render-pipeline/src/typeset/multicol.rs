...
fn split_paragraph(b: &Block, document: usize, at: usize) -> Option<(Block, Block)> {
    let Block::Paragraph { parts, indent, style, env_open, env_close, eject_before, vspace_before, addvspace_before, addvspace_flex, endlist_adjust, list, shape } = b else { return None };
...
pub(super) fn outer_doc(ctx: &mut Context, doc: &Doc, floats: &[floatpage::FloatSpec]) -> Option<Doc> {
    ...
    Some(Doc { style: doc.style.clone(), blocks: out, diagnostics: Vec::new(), limitations: Vec::new(), default_color: doc.default_color, math_colors: doc.math_colors.clone(), page_color: doc.page_color, secnumdepth: doc.secnumdepth, page_starts })
}
...
fn rec_span(ctx: &Context, r: usize) -> Option<Span> {
    match ctx.recs.get(r)? {
        ...
        // Variants added after #160 branched; each carries its own span, and
        // they are listed rather than wildcarded so the next one is caught
        // here too.
        BoxRec::ColorBox(b) => Some(b.span),
        BoxRec::Graphic(g) => Some(g.span),
        BoxRec::Transform(t) => Some(t.span),
        BoxRec::Rules { span, .. } => Some(*span),
    }
}
...
pub(super) fn paginate(ctx: &mut Context, doc: &Doc, blocks: &mut Vec<BuiltBlock>, params: &PageParams) -> Option<Vec<BuiltPage>> {
    ...
    for &(d, k, is_end) in markers.values() {
        ...
        let sub_doc = Doc { style: col_style.clone(), blocks: body, diagnostics: Vec::new(), limitations: Vec::new(), default_color: doc.default_color, math_colors: doc.math_colors.clone(), page_color: doc.page_color, secnumdepth: doc.secnumdepth, page_starts: Vec::new() };
        let laid = {
