...
pub use flashtex_font_resources::discovery::{
    default_font_dirs, default_tfm_dirs, describe_dirs, Discovery, AMS_EULER_TFM_DIR, CM_TFM_DIR, DEFAULT_FONT_DIRS,
    EC_TFM_DIR, EXE_DIR_LABEL, REQUIRED_TFM_DIR, SHARE_DIR,
};
pub use flashtex_font_resources::tfm_files::{latin_modern_outline, latin_modern_tfm, ts1_companion_tfm};
use flashtex_font_resources::tfm_files::EC_SIZES;
...
/// The EC metric file of a text role (see
/// [`flashtex_font_resources::tfm_files::ec_tfm_file`]); `None` for math.
pub fn ec_tfm_file(role: Role, size_pt: f64) -> Option<String> {
    flashtex_font_resources::tfm_files::ec_tfm_file(role.key()?, size_pt)
}

/// The OT1 metric file of a text role (see
/// [`flashtex_font_resources::tfm_files::ot1_tfm_file`]); `None` for math.
pub fn ot1_tfm_file(role: Role, size_pt: f64) -> Option<String> {
    flashtex_font_resources::tfm_files::ot1_tfm_file(role.key()?, size_pt)
}
