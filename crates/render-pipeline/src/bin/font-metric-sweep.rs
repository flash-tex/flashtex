...
fn run() -> Result<(), String> {
    ...
    // `roman_lm` (family 0 boxed from `rm-lmr*` under `lmodern`, #754) only
    // changes the TFM boxes; the sweep reads `otf_glyph`, which maps the
    // roman family to the Latin Modern text face either way. Kernel default.
    let tex = TexMathMetrics::at_text_size(SIZE_PT, false, false, std::rc::Rc::new(math_fonts), &fonts)
