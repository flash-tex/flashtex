...
impl<'a> Context<'a> {
    ...
    ///
    /// The input-encoding check ([`Self::input_filtered`]) stays in
    /// [`Self::text_box`], which is the only path a character typed in the
    /// source reaches: this one is entered directly only for a generated
    /// itemize label (`\textbullet`, `\textperiodcentered`), whose characters
    /// carry the `\item` command's span and are not input.
