...
impl MathRec {
    ...
    pub fn paint_of(&self, tag: ml::SourceTag) -> Paint {
        tag.span
            ...
            // No inner `\color` range covers this leaf: the formula's own
            // colour (#158), which is black unless xcolor set one.
            .unwrap_or_else(|| Paint::of(self.color))
    }
...
impl<'a> Context<'a> {
    ...
    fn paragraph_block(&mut self, items: &[AItem], indent: bool, starts_paragraph: bool, after_heading: bool, style: ParaStyle, list_geom: Option<&ListGeom>) -> Option<BuiltBlock> {
        ...
        if let Some(geom) = list_geom {
            ...
            if let Some(bib) = geom.bibliography.as_ref().filter(|_| starts_paragraph) {
                ...
                // #171: the item's first character protrudes into the margin
                // under microtype; the label box must not be shifted by it.
    ...
    fn label_box(&mut self, text: &str, span: Span, size: f64, bold: bool, math: bool) -> Option<(pl::GlyphRun, usize)> {
        ...
        let boxed = self.text_box_in(&seg, size, face);
    ...
    fn display_block(
        ...
        if let (None, Some(qspan)) = (&eqno, qed) {
            ...
            e = qrun.width;
            q = e + em;
            // #156 kept the symbol in a bare (run, rec) pair; this branch's
            // display_block carries the number as a NumberBox, which is also
            // where its height and depth are read from.
            eqno = Some(NumberBox { width: qrun.width, height: qrun.height, depth: qrun.depth, pieces: vec![(qrun, qrec, 0.0)] });
        }
    ...
    for (doc_index, block) in doc_blocks.iter().enumerate() {
        ...
        match block {
                ...
                for part in parts {
                    match part {
                        ...
                        ParaPart::Display {
                            ...
                            let pd = pre_display;
                            let st = *style;
                            if let Some(mut b) = ctx.cached(cache, key, origin, |c| c.display_block(list, *span, pd, number.as_ref(), *qed, st, geom)) {
            ...
            Block::Algorithmic(algorithm) => {
                ...
                // `\addvspace\@topsep`: only the excess over the glue the
                // list already ends with.
...
pub fn build_with_floats(ctx: &mut Context, doc: &Doc, cache: Option<&RenderCache>, floats: &[floatpage::FloatSpec]) -> Laid {
    ...
    let (mut built, mut images, float_labels) = if let Some(b) = multicol::paginate(ctx, doc, &mut blocks, &params) {
        ...
        // `\textheight` less the box. (#154 warned that float placement
        // could not shorten them; main's `first_colht` is what does it, so
        // the warning is dropped rather than kept alongside the fix.)
        let first_colht = top_title.as_ref().map(|t| (columns, params.vsize - t.2));
        let (mut pages, images, labels, areas) =
            floatpage::paginate(ctx, &mut blocks, &params, &list, floats, body_blocks, insertions.as_ref(), first_colht);
