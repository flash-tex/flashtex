        ...
        match resolution {
            ...
            Resolution::Fetched { version, files, source_url, notes, .. } => {
                ...
                for f in files.iter().filter(|f| f.generated_from.is_some()) {
                    eprintln!("flashtex: {} generated by docstrip from {}", f.name, f.generated_from.as_ref().map(|g| g.label()).unwrap_or_default());
                }
                print_docstrip_notes(&notes);
...
/// docstrip's notes on stderr: a batch file that programs TeX beyond
/// docstrip's commands (lipsum.ins builds its `.ltd.tex` files that way)
/// yields dozens, so only the first few are shown; all of them are in
/// the cache's `manifest.json`.
pub fn print_docstrip_notes(notes: &[String]) {
    const SHOWN: usize = 5;
    for n in notes.iter().take(SHOWN) {
        eprintln!("flashtex: docstrip: {n}");
    }
    if notes.len() > SHOWN {
        eprintln!("flashtex: docstrip: … and {} more note(s), recorded in the cache's manifest.json", notes.len() - SHOWN);
    }
}
