...
pub(crate) fn parse_fil_dimen_pt_at(text: &str, body_pt: f64) -> Option<(f64, u8)> {
    let s = text.trim();
    ...
    parse_dimen_pt_at(s, body_pt).map(|pt| (pt, 0))
}
...
impl P<'_> {
    ...
    fn hskip(&mut self, span: Span, para: &mut Vec<Inline>) {
        ...
        let Some(base_pt) = parse_dimen_pt_at(&base_text, body) else {
        ...
        for _ in 0..2 {
            ...
            match parse_fil_dimen_pt_at(&text, body) {
                Some((value, order)) => {
...
mod tests {
    ...
    fn hskip_plus_and_minus_are_recorded() {
        ...
        assert!(
            (pt - crate::layout::BODY_SIZE_PT).abs() < 1e-9,
            "base 1em, got {pt}"
        );
