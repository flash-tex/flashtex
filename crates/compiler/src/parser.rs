...
impl P<'_> {
    ...
    fn vertical_command(&mut self, name: &str, span: Span, blocks: &mut Vec<Block>, para: &mut Vec<Inline>) {
        ...
        "addvspace" => {
            ...
            // vertical-spacing model cannot express any of that: the
            // correct merge point (`page-builder`'s
            // `VListBuilder::addvspace`) is a separate crate this change
            // must not reach into. So `\addvspace{len}` unconditionally
            // inserts `len` of rigid vertical space, exactly like
            // `\vspace{len}`: a single call (with nothing but non-glue
            // material before it) matches real TeX, while two consecutive
            // calls (or an `\addvspace` right after any other vertical
            // glue) add up where real TeX would keep the larger of the
            // two.
            //
