...
impl P<'_> {
    ...
    fn vertical_command(&mut self, name: &str, span: Span, blocks: &mut Vec<Block>, para: &mut Vec<Inline>) {
        ...
        "addvspace" => {
            ...
            // vertical-spacing model cannot express that: the correct
            // merge point (`page-builder`'s `VListBuilder::addvspace`) is
            // a separate crate this change must not reach into. So
            // `\addvspace{len}` unconditionally inserts `len` of rigid
            // vertical space, exactly like `\vspace{len}`: a single call
            // matches real TeX, while two consecutive calls add up where
            // real TeX would keep the larger.
            //
