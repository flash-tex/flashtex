...
impl<'d> Converter<'d> {
    ...
    /// No partially built word, `\arraystretch` capture, or held-back
    /// `\AtBeginDocument` output: the output so far does not depend on
    /// tokens still to come. (The incremental cache only records marks and
    /// splices while clean, so a capture window is always re-converted from
    /// an earlier mark with a fresh converter.)
    fn clean(&self) -> bool {
        self.word.is_none()
