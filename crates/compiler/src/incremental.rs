...
fn shift_inlines(inlines: &mut [Inline], changes: &[ChangedBytes], deltas: &[isize]) -> Option<()> {
    for inline in inlines {
        match inline {
            ...
            Inline::NameReference { span, .. } => map_span(span, changes, deltas)?,
            Inline::AutoReference { span, .. } => map_span(span, changes, deltas)?,
...
fn block_signature(block: &Block) -> BlockSignature {
    ...
    let span_of = |inline: &Inline| match inline {
        ...
        Inline::NameReference { span, .. } => *span,
        Inline::AutoReference { span, .. } => *span,
