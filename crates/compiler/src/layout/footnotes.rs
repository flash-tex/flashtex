...
impl LayoutCursor {
    ...
    pub(super) fn footnote(
        ...
        if mark {
            // `place` pushes nothing for an empty run, so the raise below
            // must only touch the mark when one was actually placed — never
            // whatever item came before it. A wrap inside `place` can move
            // the mark onto a fresh page, so the page count is part of the
            // observation, not just the item count.
            let (pages, len) = (
                self.pages.len(),
            ...
            let pushed = self.pages.len() > pages
                || self.pages.last().is_some_and(|page| page.items.len() > len);
            if pushed {
...
mod tests {
    ...
    #[test]
    fn empty_mark_number_places_nothing_and_raises_nothing() {
        // `place` pushes no item for an empty run, so a mark with no number
        // must tolerate that empty result: no panic on an empty page, and a
        // preceding item keeps its baseline instead of taking the mark's
        // raise.
        let span = Span::new(0, 1);
        let mut cursor = LayoutCursor::new(LayoutConstraints::default());
        cursor.footnote("", span, true, None, true);
        assert!(cursor.pages.iter().all(|page| page.items.is_empty()));
        let mut cursor = LayoutCursor::new(LayoutConstraints::default());
        cursor.place("word".to_string(), 12.0, span, Font::TimesRoman, true);
        let baseline = cursor.pages[0].items[0].baseline_y_pt;
        cursor.footnote("", span, true, None, true);
        assert_eq!(cursor.pages[0].items.len(), 1);
        assert_eq!(cursor.pages[0].items[0].baseline_y_pt, baseline);
    }
