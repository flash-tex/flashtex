...
fn cite_punct(options: CiteOptions, span: Span, out: &mut Vec<Inline>) {
    match options.punct {
        ...
        // The kernel's own `,\penalty\@m\ ` shape, set as the kernel path
        // above sets it: one run whose blank is the interword space.
        CitePunct::Space => out.push(text_run(", ", span, TextStyle::default(), false)),
        CitePunct::None => {
            out.push(text_run(",", span, TextStyle::default(), false));
            out.push(Inline::Penalty { value: 1000, span, unskip: false });
        }
...
mod tests {
    ...
    /// number; `[space]` is the kernel's `,\ ` and `[nospace]` nothing after
    /// the comma. Without the package, `\cite` keeps its kernel `, `.
    #[test]
