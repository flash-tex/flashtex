...
pub const BUILT_IN_PACKAGES: &[(&str, &str)] = &[
    ...
    // algpseudocode is algorithmicx's modern, mixed-case-command bundle
    // (\State, \If, ...); algorithmic.sty above is the older, all-caps
    // sibling (\STATE, \IF). Same limitation, different package name so
    // the help text points at the \usepackage the author actually used.
