...
fn arguments_are_consumed_and_the_preface_is_body_material() {
    ...
    assert_eq!(blocks, ["P:Before", "H:Head", "P:One two.", "P:Three.", "P:After"], "{diagnostics:?}");
    assert!(diagnostics.is_empty(), "{diagnostics:?}");
}

#[test]
fn a_text_preface_ends_with_its_own_paragraph() {
    let (blocks, diagnostics) = outline(&doc("{article}", "\\begin{multicols*}{2}[Intro text] Body text.\n\\end{multicols*}"));
...
fn column_counts_are_checked_like_multicols() {
    let (_, few) = outline(&doc("{article}", "\\begin{multicols}{1}\nx\n\\end{multicols}"));
    assert!(few.iter().any(|m| m.contains("Using `1' columns doesn't seem a good idea")), "{few:?}");
    let (_, many) = outline(&doc("{article}", "\\begin{multicols}{21}\nx\n\\end{multicols}"));
    assert!(many.iter().any(|m| m.contains("Too many columns")), "{many:?}");
}

#[test]
fn columnbreak_outside_multicols_is_multicols_error() {
    let (_, diagnostics) = outline(&doc("{article}", "Text \\columnbreak more."));
    assert!(diagnostics.iter().any(|m| m.contains("\\columnbreak outside multicols")), "{diagnostics:?}");
}
...
fn the_package_is_silent_except_with_the_twocolumn_option() {
    ...
    assert!(twocolumn.iter().any(|m| m.contains("May not work with the twocolumn option")), "{twocolumn:?}");
}
