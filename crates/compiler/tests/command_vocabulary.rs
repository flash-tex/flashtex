...
fn addvspace_is_implemented_so_it_is_known_without_the_unimplemented_list() {
    // Issue #495: `\addvspace` has a real dispatch arm (max-with-pending-skip
    // in `vertical_command`) and a `BUILT_INS` entry, so it stays known for
    // typo suggestions even though it is no longer listed as unimplemented.
    // `is_known_command` alone would still pass here even if the
