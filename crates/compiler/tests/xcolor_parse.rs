...
fn color_of(parsed: &Parsed, word: &str) -> Option<String> {
    inlines(parsed)
        .into_iter()
        .find_map(|i| match i {
            Inline::Text { text, style, .. } if text == word => {
                Some(style.color.map(|c| c.fill_operator()))
            }
            _ => None,
        })
...
fn definitions_and_expressions_keep_pdftex_values() {
    ...
    assert!(p.diagnostics.is_empty(), "{:?}", messages(&p));
    assert_eq!(
        color_of(&p, "X").as_deref(),
        Some("0.05 0.09999 0.15001 0.2 k")
    );
    assert_eq!(color_of(&p, "Y").as_deref(), Some("1 0.53333 0 rg"));
    assert_eq!(
        color_of(&p, "Z").as_deref(),
        Some("0.04706 0.7843 0.12941 rg")
    );
...
fn math_takes_the_surrounding_colour_and_records_inner_ranges() {
    ...
    let maths: Vec<_> = inlines(&p)
        ...
        .filter_map(|i| match i {
            Inline::Math {
                ...
                span,
                ..
            } => Some((*color, color_ranges.clone(), *span)),
            _ => None,
        })
        .collect();
    assert_eq!(maths.len(), 2);
    assert_eq!(
        maths[0].0.map(|c| c.fill_operator()).as_deref(),
        Some("0 0 1 rg")
    );
    ...
    assert_eq!(
        ranges,
        vec![
            ("y".to_string(), "1 0 0 rg".to_string()),
            ("w".to_string(), "0 1 0 rg".to_string())
        ]
    );
}
...
fn colorbox_and_fcolorbox_capture_fill_frame_and_fboxsep() {
    ...
    assert_eq!(
        boxes[1].frame.map(|c| c.fill_operator()).as_deref(),
        Some("1 0 0 rg")
    );
...
fn page_color_and_target_models() {
    let p = doc(
        "\\usepackage[rgb]{xcolor}\\pagecolor{yellow!20}",
        "\\textcolor{cyan}{X}",
    );
    assert!(p.diagnostics.is_empty(), "{:?}", messages(&p));
    assert_eq!(
        p.page_color.map(|c| c.fill_operator()).as_deref(),
        Some("1 1 0.8 rg")
    );
    assert_eq!(color_of(&p, "X").as_deref(), Some("0 1 1 rg"));
    assert_eq!(
        p.default_color.map(|c| c.fill_operator()).as_deref(),
        Some("0 0 0 rg")
    );
...
fn errors_are_diagnosed_with_xcolor_recovery() {
    ...
    assert!(
        messages(&p)
            .iter()
            .any(|m| m.contains("needs \\usepackage{xcolor}")),
        "{:?}",
        messages(&p)
    );
    assert_eq!(color_of(&p, "a").as_deref(), Some("1 0 0 rg"));
    let p = doc(
        "\\usepackage{xcolor}",
        "{\\color{red} \\textcolor{nosuch}{b} \\textcolor[hsb]{0.5,1,1}{c}}",
    );
    let m = messages(&p);
    assert!(
        m.iter().any(|m| m.contains("undefined colour `nosuch`")),
        "{m:?}"
    );
    assert!(
        m.iter().any(|m| m.contains("`hsb` is not supported")),
        "{m:?}"
    );
