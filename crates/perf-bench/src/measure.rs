...
/// Accessors used by `warmprof`, so the warm profile drives the same request
/// bytes, the same options and the same edit as the timed warm scenarios do.
/// Anything else would be profiling a different keystroke from the one measured.
pub fn request_line(revision: usize, docs: &[(String, String)], entry: &str, caps: Caps) -> String {
    request(revision, docs, entry, caps)
}

pub fn text_at_step(base: &str, edit: Edit, step: usize) -> String {
    text_at(base, edit, step)
}

pub fn options_for(case: &Case) -> RenderOptions {
    RenderOptions { project_root: case.project_root.clone(), ..RenderOptions::default() }
}

pub fn edit_named(case: &Case, scenario: &str) -> Option<Edit> {
    edits_for(case, &[scenario.to_string()]).into_iter().next().map(|(_, e)| e)
}
...
fn decompose(docs: &[SourceDocument<'_>], case: &Case, fonts: &FontSet, options: &RenderOptions) -> Phases {
    ...
    let t = Instant::now();
    // `page_color`/`default_color` are passed exactly as `render_cached` passes
    // them (`render-pipeline/src/lib.rs`, `doc.page_color`/`doc.default_color`),
    // so this phase assembles the same display list the product path assembles.
    ...
    let t = Instant::now();
    // `device_color`, `delta` and `v2_only` are newer capabilities and are held
    // off here on purpose: the committed baselines were recorded when the struct
    // had four fields, so turning any of them on would change the work this
    // phase does and silently invalidate every recorded `v1_ms`.
    let payload = v1::fallback(
        &v2,
        Capabilities {
            rules: true,
            font_hints: true,
