...
fn decompose(docs: &[SourceDocument<'_>], case: &Case, fonts: &FontSet, options: &RenderOptions) -> Phases {
    ...
    let t = Instant::now();
    let payload = v1::fallback(&v2, Capabilities { rules: true, font_hints: true, display_list: false, images: false, device_color: false, delta: false, v2_only: false, diagnostics: false, window: false }, Some(vec![]));
    let v1_ms = ms(t);
