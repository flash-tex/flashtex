...
# per-phase split of ONE WARM KEYSTROKE (see "The warm split" below)
./target/release/flashtex-perf-bench --warm-profile synthetic-500kb \
    --warm-profile-scenario type-paragraph --steps 12
```
...
The phase table above splits a **cold** render, and a warm keystroke does not
have the same shape: the block caches turn `adapt`/`typeset`/`assemble` into
much less work while every whole-document scan and every serialisation costs
exactly what it did. `--warm-profile CASE` splits the warm keystroke instead.

It runs three passes over one scenario, each with its own `RenderCache` primed
by the identical unedited request and driven by the identical sequence of
edited documents, so all three caches evolve through the same states:

* **A** `protocol::handle_line` — the authoritative total, plus what the
  product path actually did: whether a `display_list` sibling line was emitted
  at all, and how big each reply was;
* **B** `render_cached` alone, so `A − B` is the reply cost, measured rather
  than assumed;
* **C** a seam-by-seam replay of `render_cached`'s body, to split B.

The serialisation seams are timed in pass B on the real display list, and only
the ones `handle_line` would actually have run. That distinction matters: at
500 KB the list is 164 MB against a 16 MiB line limit, so `handle_line`
declines it **without serialising it**, and an earlier version of this split
that always serialised reported a 167 ms `write_json_wire` as 57% of a
keystroke that never calls it. `coverage` is printed for the same reason —
attribute with care when it is far from 1.0.
...
invisible to this harness until `vendor/compiler` is re-pinned.** Re-pin first;
there is no flag that redirects the harness at another binary, because it calls
`protocol::handle_line` in process rather than spawning a worker.

`crates/render-pipeline` is linked directly, not vendored, so a change to the
pipeline itself — `typeset`, `adapter`, `assemble`, `incremental`, `v1` — is
measured with no re-pin.
