...
This compiler implements a finite LaTeX subset: 542 text-mode and 717 math-mode command entries, 92 environments and 43 layout-neutral packages. Every other command produces an explicit "not supported" diagnostic naming it, and every other environment or package a warning; nothing is dropped silently. Descriptions note approximations. Outstanding features with reproductions are in `crates/compiler/UNSUPPORTED.md`.
...
| `\normalmarginpar` |  | later margin notes go back to the default side after \reversemarginpar; global, in the preamble or the body |
...
| `\fbox` | `{...}` | kernel framed box: the argument with an \fboxrule frame and \fboxsep padding in the current colour (single-line) |
| `\framebox` | `[width][pos]{...}` | kernel framed box with optional fixed width and l/c/r/s position (default c); without optionals exactly \fbox (single-line; a fixed width is parsed and validated but the box is set at natural width) |
| `\makebox` | `[width][pos]{...}` | kernel unframed box with optional fixed width and l/c/r/s position (default c); without optionals exactly \mbox (single-line; a fixed width is parsed and validated but the box is set at natural width) |
...
| `\newgeometry` | `{options}` | geometry page-frame switch: ends the page like \clearpage, then applies the option string's margins; the switch position and frame are reported for the page renderer (needs geometry) |
...
| `\complexnum` | `[options]{number}` | siunitx complex number inside a formula |
| `\complexqty` | `[options]{number}{units}` | siunitx complex quantity inside a formula |
...
| `\atop` |  | TeX infix fraction with no rule inside a group |
| `\above` | `<dimen>` | TeX infix fraction with a rule of the given thickness inside a group |
| `\overwithdelims` | `<delim1><delim2><dimen>` | TeX infix fraction inside a group with outer fences: default rule, no rule, or the given thickness |
| `\atopwithdelims` | `<delim1><delim2><dimen>` | TeX infix fraction inside a group with outer fences: default rule, no rule, or the given thickness |
| `\abovewithdelims` | `<delim1><delim2><dimen>` | TeX infix fraction inside a group with outer fences: default rule, no rule, or the given thickness |
...
| `\varGamma` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varDelta` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varTheta` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varLambda` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varXi` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varPi` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varSigma` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varUpsilon` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varPhi` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varPsi` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
| `\varOmega` |  | amsmath variant capitals at cmmi10 slots 0x00-0x0A (amsmath.sty 385-395); undefined without amsmath |
...
| `questions` | text | exam question list (arabic `1.` item labels); needs \documentclass{exam} |
| `parts` | text | exam parts list (`(a)` item labels); needs \documentclass{exam} |
| `subparts` | text | exam subparts list (`i.` item labels); needs \documentclass{exam} |
| `subsubparts` | text | exam subsubparts list (greek `α)` item labels); needs \documentclass{exam} |
