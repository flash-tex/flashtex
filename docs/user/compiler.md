...
This compiler implements a finite LaTeX subset: 241 text-mode and 570 math-mode command entries, 48 environments and 10 layout-neutral packages. Every other command produces an explicit "not supported" diagnostic naming it, and every other environment or package a warning; nothing is dropped silently. Descriptions note approximations. Outstanding features with reproductions are in `crates/compiler/UNSUPPORTED.md`.
...
| --- | ---: | ---: |
| kernel | 162/410 (39.5%) | 17/30 (56.7%) |
| amsmath | 48/102 (47.1%) | 17/21 (81.0%) |
...
| `\bot` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\bigtriangleup` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\varbigtriangleup` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\vert` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\backslash` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\lvert` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\rvert` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\lVert` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\rVert` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\ldotp` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\cdotp` |  | shared symbol glyph with its own atom class (fontmath.ltx's second \DeclareMathSymbol for the same slot) |
| `\smallint` |  | small integral at cmsy10's 0.4167em advance (\int's glyph) |
| `\sqrtsign` | `{math}` | the radical \sqrt is built on, with no root index |
| `\not` |  | zero-width negation slash the following relation is overprinted with |
| `\mathring` | `{math}` | ring accent over the argument |
| `\injlim` |  | upright operator name with limits (inj lim / proj lim) |
| `\projlim` |  | upright operator name with limits (inj lim / proj lim) |
| `\varinjlim` |  | upright lim with an extensible arrow beneath |
| `\varprojlim` |  | upright lim with an extensible arrow beneath |
| `\pod` | `{n}` | parenthesised (n) after 8mu (18mu in display) |
| `\nobreakdash` |  | forbids a break after the following dash; sets no glyph |
| `\pmb` | `{math}` | argument at its own advance; the triple overprint is not reproduced |
...
| `\iiiint` |  | repeated integral glyph |
| `\idotsint` |  | repeated integral glyph |
| `\lbrace` |  | brace glyph |
| `\rbrace` |  | brace glyph |
