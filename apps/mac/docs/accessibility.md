...
| `ShellModel.previewAnnouncer` (`PreviewAnnouncements.swift`) | A completed compile is announced once: `Preview updated: 3 pages` (low priority), `Preview updated with 2 errors recovered: 3 pages`, `Compile failed: 2 errors` (high). A status flip speaks at once; same-status results under auto-compile coalesce to one announcement 2 s after the last result. A refused display list with nothing verified on screen says `Preview display list refused: …`. |
