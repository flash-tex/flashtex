        ...
        VStack(spacing: 0) {
            HStack(spacing: DS.Space.m) {
                ...
                TextField("Search files, sections, labels and commands…", text: $query)
                    ...
                    .onKeyPress(.return) {
                        ...
                        run(selectedEntry(in: entries)); return .handled
                    }
            ...
            if let jump = CommandPaletteModel.lineJumpInput(from: query) {
                ...
                VStack(alignment: .leading, spacing: DS.Space.xs) {
                    Text(preview).font(DS.Fonts.base).foregroundStyle(DS.Colors.textPrimary)
                    Text("⏎ jumps · esc closes")
                        .font(DS.Fonts.secondary).foregroundStyle(DS.Colors.textTertiary)
                }
                .padding(.horizontal, DS.Space.l).padding(.vertical, DS.Space.m)
