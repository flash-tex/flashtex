...
def pdf_page_count(path):
    """Number of pages in `path`, or None if it cannot be determined.

    The plain byte scan for `/Type /Page` only works while the page objects sit
    uncompressed in the file. Every reference generated by TeX Live 2025 (and
    any pdfTeX with `\\pdfobjcompresslevel` on, which is the default) puts them
    in an **object stream**, so the scan finds zero and this returned None --
    silently, and the callers then had nothing to compare a producer page count
    against. `pdftext` already parses xref streams, object streams and the page
    tree for the word comparison, so ask it first and keep the byte scan only
    as the fallback for a PDF it cannot parse.
    """
    try:
        import pdftext  # tools/visual-oracle, already on sys.path
        n = len(pdftext.PdfDocument.load(path).pages())
        if n:
            return n
    except Exception:  # noqa: BLE001 - any parse failure falls back to the scan
        pass
    with open(path, "rb") as f:
        data = f.read()
...
def make_request(fx, rid):
    ...
    return {
        ...
        "payload": {
            ...
            "entry_path": fx["entry"],
            "documents": docs,
            # The directory `\includegraphics` files are read from. The document
            # snapshots above carry only *text*; binary siblings (images) are
            # read from disk, rooted at this directory, and without it the
            # renderer answers every \includegraphics with the hard error
            # "no project root was supplied with the request, so image files
            # cannot be read". Leaving it out therefore did not measure image
            # handling -- it measured the harness. Absolute and resolved
            # because the worker rejects a relative or symlinked root
            # (document-runtime::validate_project_root).
            # protocol: render-pipeline/src/protocol.rs (FT-063)
            "project_root": os.path.realpath(fx["dir"]),
            # Image items are an opt-in display-list feature: without the
            # negotiated capability the renderer places the graphic but never
            # serialises it (render-pipeline/src/display.rs `Wire.images`).
...
def main():
    ...
    # No hardcoded metrics directory. The old default named
    # `texmf/fonts/tfm/public/lm` *alone*, which silently dropped the bundled
    # `jknappen/ec` (256 files, `ectt*` included) and `public/amsfonts/symbols`
    # trees, so every T1/\texttt fixture was measured on substituted roman
    # widths -- see the long note in tools/visual-oracle/fontenv.py. The
    # directories are now derived from the outline roots by walking for
    # directories that actually contain .tfm files.
    ...
    for fx in fixtures:
        ...
        for p in producers:
            ...
            if p["name"] == "render":
                # `--images`: without it flashtex-render writes the `--v2`
                # display list with `Wire { images: false }` hardcoded, so an
                # \includegraphics that resolved perfectly well still left no
                # image item in the file the PDF route reads.
                extra_args = ["--v2", v2_path, "--images"]
            ...
            if p["name"] == "render" and args.pdf_exact and os.path.isfile(v2_path) and pr["status"] in ("ok", "recovered"):
                # `--project-root`: the display list references image files by
                # project-relative path; the exporter reads their bytes itself
                # and cannot without a root.
